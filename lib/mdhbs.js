...
(function() {
  var async, baseViews, compileTemplate, fileFilter, filelet, fs, funclet, handlebars, isAbsolute, isFunction, item, loadTemplateHelper, loadTemplates, loglet, marked, monitors, path, registerHelper, registerHelpers, render, renderFile, renderKey, renderLayout, renderRelKey, templateChanged, templateCreated, templateEvent, watch, _, _renderFile,
    __slice = [].slice;
  ...
  registerHelpers = function(helpers, scope) {
    ...
    for (key in helpers) {
      ...
      if (helpers.hasOwnProperty(key) && isFunction(val)) {
        name = "" + scope + ":" + key;
        _results.push(registerHelper(name, val));
      } else {
