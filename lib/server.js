...
(function() {
  var baseErrorHandler, bean, bodyParser, cookieParser, cookieSession, express, extless, favicon, filelet, fs, http, https, initLastMiddlewares, initMiddlewares, initRoutes, localRequire, loglet, mdhbs, methodOverride, path, run, runStatic, runWithConfig, session, setupSSL, signedSecret, uuid, _;
  ...
  runStatic = function(argv) {
    ...
    if (argv.ssl) {
      return setupSSL(argv, app);
    }
  };

  runWithConfig = function(config) {
    var app, sessionConfig, _ref, _ref1, _ref2, _ref3;
    ...
    app.set('view engine', ((_ref = config.views) != null ? _ref.engine : void 0) || 'jade');
    app.engine('md', mdhbs.renderFile);
    app.engine('hbs', mdhbs.renderFile);
    app.engine('html', mdhbs.renderFile);
    app.hbs = mdhbs;
    app.addViews(path.join(config.BASE_DIR, ((_ref1 = config.views) != null ? _ref1.dir : void 0) || 'views'));
    app.addViews(path.join(config.BASE_DIR, ((_ref2 = config.views) != null ? _ref2.dir : void 0) || 'template'));
    app.use(cookieParser(((_ref3 = config.session) != null ? _ref3.secret : void 0) || signedSecret));
    sessionConfig = {
      genid: function(req) {
        return uuid.v4();
      }
